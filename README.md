...
| `update-skills.yml` | 想让云端根据 sources 自动生成或更新 skills；也会每天定时运行 | 执行 `node packages/cli/dist/index.js ai-update --layer cloud --dir .`，检查 provider 并调用 DeepSeek 更新流程 | 必需 secret：`DEEPSEEK_API_KEY`；可选 variables：`DEEPSEEK_MODEL`、`DEEPSEEK_BASE_URL` | 更新 `.agents/skills/` 中由 `skills-manage` 管理的 skills；当前 v1 仍在补完整生成落盘流程 |
